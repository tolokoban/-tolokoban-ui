...
exports.cssForGaps = exports.cssForColorOn = exports.cssForColor = void 0;
function cssForColor(name, alpha = 1) {
    if (alpha <= 0)
        return "transparent";
    if (alpha >= 1)
...
exports.cssForColor = cssForColor;
function cssForColorOn(name) {
    return `var(--theme-color-on-${name})`;
}
exports.cssForColorOn = cssForColorOn;
function cssForGaps(params, unit = "em") {
    if (!Array.isArray(params))
        return cssForGap(params, unit);
    return params.map((item) => cssForGap(item, unit)).join(" ");
}
exports.cssForGaps = cssForGaps;
